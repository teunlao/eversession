...
function modeFromThreshold(tokens: number | undefined, threshold: number | undefined): StatusMode {
  if (tokens === undefined || threshold === undefined) return "Active";
  return tokens >= threshold ? "Over" : "Waiting";
}
...
async function buildClaudeStatusReport(params: {
  ...
  return {
    ...
    mode,
    tokens: { ...(displayTokens !== undefined ? { current: displayTokens } : {}), ...(threshold !== undefined ? { threshold } : {}), ...(bar ? { bar } : {}) },
  };
}
...
export function registerStatusCommand(program: Command): void {
  program
    ...
    .action(async (opts: StatusCommandOptions) => {
      ...
      const envCodexSessionId = process.env.EVS_CODEX_SESSION_ID?.trim();
      const effectiveSessionId =
        opts.sessionId ?? (agentRaw === "codex" || agentRaw === "auto" ? envCodexSessionId : undefined);
      ...
      const report = await resolveSessionReportForStatus({
        ...
        maxCandidates:
          Number.isFinite(maxCandidatesRaw) && maxCandidatesRaw > 0 ? Math.floor(maxCandidatesRaw) : 200,
